...
def encode_layout(boxes, labels, no_sections=90, max_objects=30, pad_value=8191, dummy_category=0):
    ...
    device = boxes.device
    idxs = torch.linspace(0, labels.shape[1]-1, labels.shape[1], device=device).long()
    ...
    extra = torch.tensor(token_pair_from_bbox(torch.tensor([0,0,1,1]), no_sections=no_sections)).to(cond.device).unsqueeze(0)
